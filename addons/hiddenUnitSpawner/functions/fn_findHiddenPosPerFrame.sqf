...
if (_players isEqualTo [] || _nearObjects isEqualTo []) exitWith {
	...
	[
		{_this call AF_fnc_findHiddenPos},
...
private _posInBlacklist = [_objPos] call AF_fnc_mapPosCheck;
if (_posInBlacklist) exitWith {
	[
		{_this call AF_fnc_findHiddenPosPerFrame},
...
{
	if ([_x] call AF_fnc_checkNearDeadUnit) exitWith {
...
// check pos
private _averagePos = [_players] call AF_fnc_getAveragePos;
...
[_newUnit, _tickets - 1] call AF_fnc_hiddenPosEH;