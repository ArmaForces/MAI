...
if (_useAnimation) then {
	[_unit,"AmovPsitMstpSrasWrflDnon_AmovPercMstpSlowWrflDnon"] remoteExecCall ["switchMove", 0];
};
_unit setVariable ["acex_headless_blacklist", true, 0];
{[_x,[[_unit],true]] remoteExecCall ["addCuratorEditableObjects", 0]} forEach allCurators;
