		...
		if (alive _unit) then {
			...
			if !(_vehicle isKindOf "MAN") then {
				...
				if (_vehicleIndex == -1) then {
					private _vehInfo = _vehicle call FUNC(getVehicleInfo);
			...
			if (_objects isEqualTo []) then {
				_objects = lineIntersectsWith [_posASL vectorAdd [0,0,0.5], _posASL vectorAdd [0,0,-3.5], _unit, vehicle _unit, true];
			};
			if !(_objects isEqualTo [])then {
